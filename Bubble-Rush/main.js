document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    showPopup(); // Show the popup when the ESC key is pressed
  }
});
...
document.getElementById("cancelBtn").addEventListener("click", () => {
  hidePopup(); // Close the popup without doing anything
});
...
async function Draw() {
        ...
        B.map((v, i) => {
          ...
          B.map((q, j) => {
            if (j != i) {
              ...
              d =
                1 +
                (Math.hypot(X - X2, Y - Y2, Z - Z2) * (3 + iBc / 40) * 3) ** 4;
              X += ((X - X2) * 99) / d;
              Y += ((Y - Y2) * 99) / d;
              Z += ((Z - Z2) * 99) / d;
            }
          ...
          for (let i = cl; i--; ) {
            if (parts > 1) {
              ...
              p2b =
                ...
                Math.acos(
                  (ay2b - ay1b) /
                    (Math.hypot(ax2b - ax1b, ay2b - ay1b, az2b - az1b) + 0.001)
                );
            }
            a = [];
          ...
          b = [
            ...
            [
              (a = [
                ...
                C(p),
                2 ** 0.5 / 2,
              ])[j % 3] * (l = j < 3 ? size / 2 ** 0.5 : -size / 2 ** 0.5),
              a[(j + 1) % 3] * l,
              a[(j + 2) % 3] * l,
            ],
          ];
    ...
    checkCompletions = (X, Y, Z, id) => {
      ...
      B.map((v, i) => {
        if (id == v[6] || v[6] < 0) {
          ...
          ty = v[1];
          tz = v[2];
          if ((d = Math.hypot(tx - X, ty - Y, tz - Z)) < iBr * 1.1 && d > 0.9) {
            if (v[6] < 0) {
                ...
                switch (v[6]) {
                  case -2:
                    ...
                    B.map((q, j) => {
                      if (
                        q[1] > -6 &&
                        Math.hypot(tx - q[0], ty - q[1], tz - q[2]) < iBr * 4
                      ) {
                        cull = [...cull, j];
                      }
                    });
                  ...
                  case -1:
                    ...
                    B.map((q, j) => {
                      if (
                        q[1] > -6 &&
                        Math.hypot(tx - q[0], ty - q[1], tz - q[2]) < iBr * 2
                      ) {
                        cull = [...cull, j];
                      }
                    });
  ...
  x.strokeStyle = "#000";
  x.lineWidth = 10;
  x.strokeText(`score      ${score}`, c.width * 0.6, c.height - fs / 3 - fs);
  x.fillText(`score      ${score}`, c.width * 0.6, c.height - fs / 3 - fs);
  x.strokeText(`high-score ${highScore}`, c.width * 0.6, c.height - fs / 3);
